...
class DataManager():
    ...
    def load_metadata(self):
        ...
        song_infos_sorted = sorted(
            [(info['id'], info['count'], info['duration_ms']) for info in self.tracks_info.values()])
    ...
    def get_pop_bucket(self, x):
        ...
        if (type(x) == torch.Tensor):
            buckets = 1 + torch.div(100 * torch.log(x / 2), np.log(MAX_POP / 2),
                                    rounding_mode='trunc')  # 2 -> 1 and MAX_POP -> 101
        else:
    ...
    def prepare_charts(self):
        ...
        self.ordered_tracks = [e[0] for e in sorted({v["id"]: v["count"] for k, v in self.tracks_info.items()}.items(),
    ...
    def split_sets(self):
        ...
        candidate_indices = rng.choice(
            list(set(playlist_track_csc.indices[playlist_track_csc.data > 2 * self.min_songs_test])),
            2 * self.test_size, replace=False)  # find all playlists that have at least 10 songs
    ...
    def get_ground_truth(self, set_name, binary=True, resplit=False, n_start_songs=False):
        ...
        data = self.playlist_track[
            indices[1000 * (n_start_songs - 1): 1000 * n_start_songs]]  # select 1000 tracks for this configuration
        ...
        for i in range(data.shape[0]):
            ground_truth_list.append(
                set(ground_truth_array.indices[ground_truth_array.indptr[i]:ground_truth_array.indptr[i + 1]]))
            ground_truth_list_first.append(
                set(ground_truth_first.indices[ground_truth_first.indptr[i]:ground_truth_first.indptr[i + 1]]))
