...
class CompletionModel():
    ...
    def complete_val_playlists(self, n_recos=500, batch_size=-1, n_seed=5):
        ...
        val_input = self.data_manager.binary_val_sets[n_seed].indices.reshape(
            (self.data_manager.binary_val_sets[n_seed].shape[0], n_seed))
...
class MatrixFactorizationModel(CompletionModel):
    ...
    def __init__(self, data_manager, foldername="resources/data/embeddings", emb_size=128, retrain=False,
                 name='mf_model'):
    ...
    def prepare_item_factors(self, retrain=False):
        if retrain:
            ...
            als_model = implicit.als.AlternatingLeastSquares(factors=self.emb_size, calculate_training_loss=True,
                                                             iterations=10, regularization=12, use_gpu=use_gpu,
                                                             use_cg=True, use_native=False, )
