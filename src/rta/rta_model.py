...
class RTAModel(torch.nn.Module):
    ...
    def prepare_training_objects(self, tuning=False):
        ...
        train_dataset = SequentialTrainDataset(self.data_manager, train_indices,
    ...
    def beam_search(self, X_seed, all_rep, n_recos, beam_size, lv=- 10 ** 3, debug=False):
        ...
        X_beam = torch.cat(
            [X_seed.unsqueeze(2).repeat(1, 1, beam_size).reshape((beam_size * bs, -1)), first_preds.to(dev)],
            dim=1)
        if debug:
        ...
        while l < n_seed + n_recos:
            ...
            X_beam = torch.cat(
                [X_beam.reshape((bs, beam_size, -1)).gather(dim=1, index=seqs.unsqueeze(2).repeat((1, 1, l))).reshape(
                    (-1, l)),
