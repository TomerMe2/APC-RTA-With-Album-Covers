...
class AttentionFMRepresenter(BaseEmbeddingRepresenter):
    def __init__(self,
        ...
        decoder_layer = CustomTransformerDecoderLayer(d_model=emb_dim, nhead=n_att_heads, dim_feedforward=1024,
                                                      activation='gelu', dropout=dropout_att, batch_first=True)
    ...
    def attention_FM(self, E):
        ...
        E_agg = torch.transpose(self.linear(torch.transpose(E_attention, 2, 1)), 2, 1).view(
            (-1, origin_shape[1], origin_shape[-1]))
        return E_agg
