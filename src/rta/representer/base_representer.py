...
class BaseEmbeddingRepresenter(nn.Module):
    ...
    def __init__(self,
        ...
        self.album_embedding = nn.Embedding.from_pretrained(torch.tensor(album_enhanced_embedding),
                                                            freeze=False).float()
        ...
        self.artist_embedding = nn.Embedding.from_pretrained(torch.tensor(artist_enhanced_embedding),
                                                             freeze=False).float()
        ...
        self.pop_embedding = nn.Embedding.from_pretrained(
            torch.tensor(np.load(data_manager.pop_embeddings_path)[:, :emb_dim]), freeze=False).float()
        ...
        self.dur_embedding = nn.Embedding.from_pretrained(
            torch.tensor(np.load(data_manager.dur_embeddings_path)[:, :emb_dim]), freeze=False).float()
