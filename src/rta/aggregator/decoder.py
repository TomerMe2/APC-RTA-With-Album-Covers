...
class DecoderModel(AggregatorBase):
    def __init__(self,
        ...
        decoder_layer = CustomTransformerDecoderLayer(d_model=embd_size, nhead=n_head, dim_feedforward=4096,
                                                      activation='gelu', dropout=drop_p)
        ...
        if self.use_position:
            if seq_len < self.max_len:
                pos_embeddings = self.position_embedding(
                    torch.Tensor(range(self.max_len - seq_len, self.max_len)).long().to(get_device())).unsqueeze(
                    1).repeat((1, bs, 1))
                X = X + pos_embeddings
            else:
